...
import static com.xx.lxz.api.MessageCode.GETCHECKORDERLIST;

/**
 * Created by chenyf45 on 2018/5/29.
 */

public class CheckOrderFragment extends BaseFragment implements OnRefreshListener, OnLoadMoreListener {
...
//                                List<Order.data.OrderMode.OrderDetailsMode> detailsDates=order.getData().getList().get(i).getPaymentList();
                                ...
                                List<Order.data.OrderMode.OrderDetailsMode> detailsDates=new ArrayList<>();
                            ...
                            OrderAdapter adapter = new OrderAdapter(mContext, dataList,"2");
                            refreshRecyclerView.setAdapter(adapter);
    ...
    public void onEventMainThread(RefreshtEvent event) {
        ...
        if(event.getMrefreshPosition()!=null){
            if(event.getMrefreshPosition().getActive().equals(GlobalConfig.ACTIVE_PAY)){
                if(event.getMrefreshPosition().getPosition().equals(GlobalConfig.REFRESHPOSITIO_ORDER_CHECK)){
