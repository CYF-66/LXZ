...
import com.xx.lxz.bean.ContactBean;
...
public class ContactActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv_title.setText("联系人信息");
        customProgressDialog = new CustomProgressDialog(mActivity);
//        List<String> steps = Arrays.asList(new String[]{"身份认证","信息认证", "联系人信息", "","",""});
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_save://
                ...
                if(TextUtils.isEmpty(relative1Sex)){
                    ToastUtil.ToastShort(mActivity,"亲属1性别不能为空");
                    return;
                }
                if(TextUtils.isEmpty(relative1Phone)){
                    ToastUtil.ToastShort(mActivity,"亲属1手机号不能为空");
                    return;
                }
                if(TextUtils.isEmpty(relative1)){
                    ToastUtil.ToastShort(mActivity,"亲属1关系不能为空");
                    return;
                }
                ...
                if(TextUtils.isEmpty(relative2Sex)){
                    ToastUtil.ToastShort(mActivity,"亲属2性别不能为空");
                    return;
                }
                if(TextUtils.isEmpty(relative2Phone)){
                    ToastUtil.ToastShort(mActivity,"亲属2手机号不能为空");
                    return;
                }
                if(TextUtils.isEmpty(relative2)){
                    ToastUtil.ToastShort(mActivity,"亲属2关系不能为空");
                    return;
                }
...
//                        jsonobject.put("id", product_id);
                        ...
                        String result = HttpService.httpClientPost(mActivity, HttpConstant.GETRELATIVEINFO, jsonobject);
        ...
        if (!NetUtil.checkNet(mActivity)) {
            ...
            class StartThread extends Thread {
                ...
                public void run() {
                    ...
                    try {
                        JSONObject jsonobject = new JSONObject();
                        jsonobject.put("family_linkone_name", family_linkone_name);
                        jsonobject.put("family_linkone_sex", family_linkone_sex);
                        jsonobject.put("family_linkone_phone", family_linkone_phone);
                        jsonobject.put("family_linkone_relation", family_linkone_relation);
                        jsonobject.put("family_linkone_address", family_linkone_address);
                        jsonobject.put("family_linktwo_name", family_linktwo_name);
                        jsonobject.put("family_linktwo_sex", family_linktwo_sex);
                        jsonobject.put("family_linktwo_phone", family_linktwo_phone);
                        jsonobject.put("family_linktwo_relation", family_linktwo_relation);
                        jsonobject.put("family_linktwo_address", family_linktwo_address);

                        String result = HttpService.httpClientPost(mActivity, HttpConstant.CUSTOMRELATIVEINFO, jsonobject);

                        Message msg = handler.obtainMessage();
                        msg.what = MessageCode.SAVECUSINFO;
                        msg.obj = result;
                        msg.sendToTarget();
                    } catch (Exception e) {
                        e.printStackTrace();
